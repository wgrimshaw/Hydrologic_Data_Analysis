...
> Watersheds that feed rivers.
> Small scale river mapping.
...
A feature (object) most often falls into one of these three categories (more complex examples exist; see the `sf` package (simple features) documentation for more): 

* **Point** just a coordinate pair, with maybe a measurement
* **Line** great for connectivity between points
* **Polygon** something with area
...
maine.spatial <- st_as_sf(secchiMaine, coords = c("long", "lat"), crs = 4326)
...
```{r}
plot_grid(Maineplot, MaineDepthPlot, nrow = 1)
```
