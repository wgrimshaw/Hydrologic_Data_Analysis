...
> aggregation: Statistics for a time interval, maybe interested in range over a day rather than individual sample points. Also if we know there is seasonality to data, may want to aggregate on that recurrence or seasonal frequency.

> subsampling: Comparison across two different sampling frequencies. May not want to aggregate, as it would smoothe out the data, so might only subsample.

> interpolation: Greater irregularity to missing data.
...
* **modified Mann-Kendall**: no seasonality, non-parametric, *accounts for temporal autocorrelation*, missing data allowed. Function: `mmky` and `mmkh` (package: modifiedmk). More conservative in looking for trend than the Mann-Kendall because accounts for autocorrelation.
...
# what information is available for Lake Mendota? Use to find lagoslakeid
...
Mendotadata <- Mendotadata %>%
  ...
  arrange(sampledate) # sort by date
...
* **Piecewise constant**: Use nearest neighbor, creating a stepwise function.
* **Linear**: Just directly connect neighboring points to calculate average.
* **Spline**: <fill in notes here>
* **Seasonal Interpolation**: Use previous years to predict a time of year.
...
# note that approx() does not automatically create a data frame
linearinterpolation <- as.data.frame(approx(Mendotadata, n = 303, method = "linear"))
# the first column reset to number of days since 1970 and needs to be corrected
...
# Run SeasonalMannKendall test, from 'trend' package
...
# use sen's slope to estimate slope of trend
sens.slope(Mendotatimeseries)

```

What would we conclude based on these findings? Describe as you would to an educated but non-specialist audience.

> There is NOT a statistically significant trend over time. S is a cumulative sum of the difference between neighboring points.

> The summary gives the trend or possible trend for each season over time. The summary indicates that there is a statistically significant decrease in total phosphorus in October and November.
