...
> Variable 1: Secchi Depth

> Variable 2: Chlorophyll A

> Variable 3: Total Organic Carbon
...
LAGOSlanduse$lagoslakeid <- as.factor(LAGOSlanduse$lagoslakeid)
LAGOSnutrient$lagoslakeid <- as.factor(LAGOSnutrient$lagoslakeid)
LAGOSlocus$lagoslakeid <- as.factor(LAGOSlocus$lagoslakeid)

## from landuse, only keep row crops 82, pasture 81 for each set of years and the lake id

LAGOS_crops_pasture <- LAGOSlanduse %>%
  select(lagoslakeid,
         buffer500m_nlcd1992_pct_81,
         buffer500m_nlcd1992_pct_82) %>%
  mutate(Crop = buffer500m_nlcd1992_pct_82) 

LAGOSnitrogen <- left_join(LAGOSnutrient, LAGOSlocus, by = "lagoslakeid")

LAGOSnitrogen <-
  left_join(LAGOSnitrogen, LAGOS_crops_pasture, by = "lagoslakeid") %>%
  select(lagoslakeid, sampledate, chla, tn, secchi, toc,
         gnis_name, Crop) %>%
  ...
  drop_na(chla:toc) ## remove entire row if any of the three columns have NA values
```
...
```{r}
LAGOSnitrogen.cor <- LAGOSnitrogen %>%
  select(-sampledate, -gnis_name)
LAGOS.cor <- cor(LAGOSnitrogen)
corrplot(LAGOSnitrogen, method = "ellipse")
```
