...
options(scipen = 100) ## how many decimal places we want to show before displaying as eXXXX

lagosne_get(dest_folder = LAGOSNE:::lagos_path(), overwrite = TRUE) ## only have to do once on a computer to get data to local drive
...
LAGOSlocus <- LAGOSdata$locus ## metadata about each lake
LAGOSstate <- LAGOSdata$state ## simple metadata for each state with lake data
LAGOSnutrient <- LAGOSdata$epi_nutr # nutrient data only available for the epilimnion
...
LAGOStrophic <- 
  ...
  ## function quarter() knows to look at the month part of sampledate
  drop_na(chla:secchi) ## remove entire row if any of the three columns have NA values

## quarter function assigned numbers (as factors) to the seasons
...
> seasonality has a greater impact on chlorophyll concentration that total P because sunlight and P are needed for phytoplankton growth.
...
# use colormap() to show color palette and choose colors
```
...
> Generally, chlorophyll predicts a higher trophic state than secchi depth. Chlorophyll also generally predicts higher trophic state than total phosphorus.Interesting relationship where chlorophyll is between 40 and 60 but phosphorus is from 40 to 5ish, possibly when biomass is taking up phosphorus very quickly.
...
> Violin: range and distribution of the 

> Bar: sampling frequency in addition to rough distribution information

> Jitter: seasonal trends
...
Saving Lagostrophic as csv

```{r}
write.csv(x = LAGOStrophic, file = "./Data/Processed/LAGOStrophic.csv",
          row.names = FALSE)
```
