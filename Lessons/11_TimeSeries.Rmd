...
## use the word cyclical to refer to longer time scales than seasonal time scales
```
...
> No, it is essentially just drawing an average line.
...
> Data without seasonal trends, such as annual means, or a specific day of the year across time.
...
## must be regularly spaced data, in this case daily time set
## check with table(diff(ClearCreekDischarge$Date))
## may need to interpolate to fill in gaps if present
