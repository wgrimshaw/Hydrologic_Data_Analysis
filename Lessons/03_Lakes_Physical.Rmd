...
** Lotic = flowing water
** Lentic = still water
** Wetlands generally have emergent vegetation
What are the physical properties of lakes?
** little flow in comparison to volume
** slow enough flow that we think of it as not flowing
** Land locked
## Session Set Up
```{r, message = FALSE}
...
dim(NTLdata)
NTLdata <- NTLdata %>%
  drop_na(temperature_C) ## specifies only the temperature column
dim(NTLdata)  
...
## Summary gives summary statistics for numerics, not all the different numbers
```
...
<add notes here>
There is greater variation at the surface. Both the variation decreases and the temperature decreases with depth.
...
Here are are highlighting depths considered the **metalimnion** or **thermocline**. Rapid temperature decline with depth.
```{r}
...
TempprofileMay2016 <- 
  ...
  geom_vline(xintercept = 4, lty = 2) + ## 4 degrees in maximum density of water
  scale_y_reverse(breaks = c(0, 2, 4, 6, 8, 10, 12)) + ## to flip y axis
...
```
Epilimnion, then Thermocline, then Hypolimnion.
...
* Two mixing times throughout the year, spring and fall. When the column is mixed, the temperature is relatively consistent throughout.
...
Need a small enough density gradient that the wind can cause mixing throughout the water column. This generally happens in the spring and fall.
