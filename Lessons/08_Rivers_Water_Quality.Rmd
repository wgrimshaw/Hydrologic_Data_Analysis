...
> Indicator species are species that may be relatively easy to monitor that indicate the overall quality of a body of water without needing to measure specific constituents. If we are worried about water quality because of its impact on the ecosystem, an indicator species may give a more direct measure of the outcome of interest than many other parameters.

> May have wanted a discussion of indicator bacterial species, which are species that may indicate the presence of other, harder to measure, species.
...
> Differences in local resources, time intensiveness.

Where and when do you think the biggest risks from harmful bacteria are?

> After floods, during rainy seasons in developing countries.
...
> Cannot tell from the plots created. Need to be able to more directly compare the parameters, as the parameters measured changed over time.
...
> This water absolutely needs to be treated for bacterial contamination. There are peaks thoughout the record far above the drinking water standard.
...
> Near the end of the summer and early fall. WWTP discharges might be greatest in comparison to the natural stream discharges in these months. Alternatively, if this is when there is more precipitation, there may be CSOs or the WWTPs may be running above capacity.
...
> The site within the same distance of Lowell with a smaller drainage area, so the site on the concord river.

What would a CSO event look like in the data?

> High discharge at the same time as high bacterial count.
...
## jan 17 1973
begin.date <- as.Date("1973-01-01", format = "%Y-%m-%d")
end.date <- as.Date("1973-01-31", format = "%Y-%m-%d")
  
discharge.plot <- ggplot(data = discharge.dat %>% 
                   filter(Date > begin.date & Date < end.date), 
                 aes(x = Date, y = Discharge_cfs)) +
            geom_line() +
            scale_x_date(limits =  c(begin.date, end.date), name  = "date")
  
microb.plot <- ggplot(data = microb.dat %>% 
                   filter(), 
                 aes(x = Date)) +
            geom_point(aes(y = Total.coli))+
            scale_x_date(limits =  c(begin.date, end.date), name = "date")
  
  
plot_grid( discharge.plot, microb.plot, ncol = 1)
