...
> Increased variability and unpredictability of discharge/storms. Decreased snowpack from increased temperatures and less predictability of when the snowpack melts.

> Impacts ecology, hydropower generation, irrigation, flood mitigation.
...
## Note that the data begin in September, so the first year is incomplete
...
EnoRecurrence <- 
  ...
  mutate(Rank = rank(-PeakDischarge), # rank normally gives largest rank to largest number
...
> The smaller period of record underestimated the magnitude of discharges with long recurrence intervals.
...
> Ask an extremely old resident of the area the five largest storms that they remember and how they compare to a recent storm.

> Use longest record available and possibly records from nearby streams, including a trend analysis to see if the discharge is stationary.
...
SaladoRecurrencePlot.both <- 
  ggplot(SaladoRecurrence.old, aes(x = RecurrenceInterval, y = PeakDischarge)) +
  geom_point() +
  geom_point(data = SaladoRecurrence.recent, color = "#02818a",
             aes(x = RecurrenceInterval, y = PeakDischarge))
print(SaladoRecurrencePlot.both)
...
Salado.RImodel.old <-
  lm(data = SaladoRecurrence.old, PeakDischarge ~ log(RecurrenceInterval))
summary(Salado.RImodel.old)

Salado.RImodel.recent <-
  lm(data = SaladoRecurrence.recent, PeakDischarge ~ log(RecurrenceInterval))
summary(Salado.RImodel.old)
