...
#install.packages("streamMetabolizer", dependencies = T,
...
> Chemostatic means same chemical concentrations at differing flows. Crane Creek is more chemostatic than the Potomac.
...
Diluting means negative relationship between C and Q. Flushing is positive relationship between C and Q. Chemostatic is no relationship. Clockwise loop if 
...
## nitrate concentration decreases with flow during storms
```
...
> Counterclockwise hysteresis. Rising limb has a lower concentration than the falling limb of the storm
...
> How quickly a stream consumes organic matter

Why would we want to know the *GPP*, gross primary productivity, and *ER*, Ecosystem respiration, of a river reach?

> GPP - produce oxygen while fixing carbon to grow biomass
> 
> How much biomass, etc an ecosystem can support
...
```{r}
# Use streamPULSE to create list of data, site information, and specs
Enodat <- request_data(
  sitecode = "NC_Eno",
  variables = c('DO_mgL'),
...
Eno.lon <- Enodat[[2]]$lon # variable corresponding to Eno longitude
...
> Production is highest during the day because it needs photosynthesis. Respiration occurs roughly equally all the time. Dissolution 
